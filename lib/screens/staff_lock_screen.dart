...
class _StaffLockScreenState extends ConsumerState<StaffLockScreen>
  ...
  @override
  void didUpdateWidget(StaffLockScreen old) {
    super.didUpdateWidget(old);
    // _AuthGate resolves shopId asynchronously (SharedPreferences).
    // When it arrives and triggers a rebuild with a non-empty shopId,
    // reload staff so the list appears without needing a hot restart.
    if (widget.ownerShopId.isNotEmpty &&
        widget.ownerShopId != old.ownerShopId) {
      WidgetsBinding.instance.addPostFrameCallback((_) {
        if (mounted) {
          ref.read(staffProvider.notifier).reloadIfEmpty(widget.ownerShopId);
        }
