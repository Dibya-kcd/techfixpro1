// ─────────────────────────────────────────────────────────────────────────────
//  screens/staff_lock_screen.dart  — Modern redesign
//
//  UX Philosophy:
//  • Clean, focused "Who's working?" lock screen — staff-first experience
//  • Owner access is HIDDEN when staff exist (5-tap logo → bottom sheet)
//  • When NO staff exist, a visible "Owner Access" button is shown so the
//    owner isn't locked out of a fresh/empty shop
//  • Smooth numpad PIN entry with animated dots
//  • Staff avatars with initials + role chip
//  • Firebase Auth stays connected — DB streams stay live at all times
// ─────────────────────────────────────────────────────────────────────────────
...
import '../data/providers.dart'; // settingsProvider
...
class _OwnerAccessSheetState extends ConsumerState<_OwnerAccessSheet> {
  ...
  Widget _buildVerifyView() {
    return Column(
      ...
      children: [
        // Mode toggle — PIN only when ownerUid is known (already authed)
        Row(
          children: [
            if (_canUsePin) ...[
              _ModeChip(
